...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    labels = tf.reshape(samples[common.LABEL], shape=[-1]) ######### gt
    ...
    weights = tf.squeeze(tf.where(tf.less_equal(labels, dataset.num_classes - 1)), 1)  # ignore all labels >= num_classes ##/////////////////
    labels = tf.cast(tf.gather(labels, weights), tf.int32) ##/////////////////
    predictions = tf.gather(predictions, weights) ##/////////////////
    ...
    metric_map = {}
    metric_map[predictions_tag] = tf.contrib.metrics.streaming_mean_iou(predictions, labels, num_classes=dataset.num_classes) ##/////////////////
